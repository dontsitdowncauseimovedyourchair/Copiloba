...
# Add /home/root to sys.path so the voice/ package is importable when running
# from any working directory.
sys.path.insert(0, "/home/root")
from voice.assistant import VoiceAssistant       # audio pipeline
from voice.command_router import CommandRouter   # action dispatcher
...
class MapScreen(Gtk.Overlay):
    ...
    def __init__(self, nav_callback):
        ...
        # ── FIX: add map directly as the Overlay base child ──────────────────
        # Previously, both the map and the UI box were placed inside a
        # Gtk.Fixed as the single base child.  OsmGpsMap.Map expands to fill
        # the entire Fixed container and paints over everything else inside it,
        # making the UI controls invisible.
        # The correct GTK.Overlay pattern (same as CameraScreen and
        # MusicScreen) is:
        #   self.add(map_widget)       → base layer, fills window
        #   self.add_overlay(ui_box)   → floats on top, transparent background
        ...
        # ── FIX: use add_overlay so ui_box floats above the map ─────────────
...
# ─────────────────────────────────────────────
# 9. MIC WIDGET (Voice Assistant UI)
# ─────────────────────────────────────────────
class MicWidget(Gtk.Box):
    """
    A vertical box containing:
      - A microphone button (hold to talk / tap to talk)
      - A small status label ("Escuchando…", "Pensando…", etc.)

    The widget wires itself to a VoiceAssistant instance.
    The owner must call set_assistant(assistant, router) after construction.

    Layout position on HomeScreen: bottom-right of the dock area.
    """

    # Human-readable labels for each pipeline state
    _STATE_LABELS = {
        "idle":         "",
    ...
    def __init__(self):
        ...
        # Mic button — uses image if mic.png exists, falls back to emoji
        self._btn = Gtk.Button()
        self._btn.get_style_context().add_class("mic-button")
        self._btn.set_size_request(80, 80)
        self._load_icon()
        self._btn.connect("clicked", self._on_click)

        # Status label
        self._lbl = Gtk.Label(label="")
        self._lbl.get_style_context().add_class("mic-status")
    ...
    def set_assistant(self, assistant, router):
        """Called by CarPlayWindow after both objects are created."""
        self._assistant = assistant
        self._router    = router

        assistant.on_state_change = self._on_state_change
        assistant.on_command      = self._on_command
        assistant.on_error        = self._on_error

    def _load_icon(self):
        mic_path = "/home/root/media/mic.png"
        try:
            pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(
                mic_path, 42, 42, True
            )
            self._btn.set_image(Gtk.Image.new_from_pixbuf(pixbuf))
        except Exception:
            # Fallback: unicode emoji label
            self._btn.set_label("🎙")
    ...
    def _on_click(self, widget):
        ...
        if self._is_recording:
            ...
            self._assistant.stop_recording()
        else:
            self._assistant.start_recording()

    # ── Callbacks (already on GTK thread via GLib.idle_add) ──────────────────

    def _on_state_change(self, state):
        ctx = self._btn.get_style_context()
        # Clear dynamic classes first
        for cls in ("recording", "busy"):
        ...
        label = self._STATE_LABELS.get(state, "")
    ...
    def _on_error(self, msg):
        self._lbl.set_text(f"⚠ {msg}")
        # Clear after 4 seconds so the label doesn't linger forever
        GLib.timeout_add(4000, self._clear_error)
...
class CarPlayWindow(Gtk.Window):
    def __init__(self):
        ...
        # ── Voice assistant (Phase 1-3) ───────────────────────────────────────
        # Instantiate after all screens exist so CommandRouter can reference them.
        # MicWidget was already added to the HomeScreen fixed layer by
        # _build_home(); we wire the assistant to it here.
        self._voice_assistant = VoiceAssistant()
        self._command_router  = CommandRouter(self)
        self._mic_widget.set_assistant(self._voice_assistant, self._command_router)
    ...
    def _build_home(self):
        ...
        # ── Mic widget — voice assistant button ──────────────────────────────
        # Stored as self._mic_widget so CarPlayWindow.__init__ can wire the
        # VoiceAssistant after all screens are constructed.
        self._mic_widget = MicWidget()
        fixed.put(self._mic_widget, 710, 510)
